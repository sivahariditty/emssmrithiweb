(function ($) {
    ...
    $(document).bind('erf_process_form', function (ev, selector, submission) {
         ...
         * 1. Disable all the Primary Fields (User Email,Password,Username or field with `user` entity property).
         * 2. Empty primary field values and placeholder values.
         * 3. Disable `User role` selection 
         */
        if (erf_global.logged_in == 1) {
            ...
            primaryFields.attr('placeholder', '');
        ...
        formContainer.each(function () {
            ...
            form.find('.fb-file input[type=file]').each(function(){
                $(this).on('input',function(event){
                    if(event.target.files.length==0) return;
                    if(window.FileReader){} else {return;}
                    var imageFile= event.target.files[0];
                    var fileContainer= $(this).closest('.fb-file');
            ...
            form.find('.fb-select select option:not([value])').val('');
    ...
    $(document).bind('erf_process_form_dynamic_fields', function (ev, form, submission, formData, userMeta) {
        ...
        var applyConfiguration = function (data) {
            ...
            if (data.hasOwnProperty('tel_config')) {
                var tels = form.find('[type=tel][enable-intl=true]');
                if (tels.length > 0) {
                    tels.each(function () {
    ...
    /*****************************Utils ****************/
    var show_loader= function(btn,state){
    ...
    var attachFileWithField = function(container,attachment,submissionID,allowDeletion){
        ...
        if(attachment.hasOwnProperty('image_url')){
            var fileContainer= attachment_field.closest('.fb-file');
            var previewContainer= $('<div/>', {class: 'erf_img_preview file_preview'});
            var preview= $('<img/>');
            previewContainer.append(preview);
        ...
        if(file_exists){
            ...
            attachment_field.closest('.fb-file').find('.fb-required').hide();
            if(allowDeletion){
                ...
                deleteFile.click(function(e){
                    ...
                    if(isRequired){ // Adding required attribute if field is required.
                        ...
                        attachment_field.closest('.fb-file').find('.fb-required').show();
                    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    try {
                        ...
                        show_loader(submitButton,false);
                    } catch (ex) {
                    }
                },
            });
            ...
            $.post(erf_global.ajax_url, {form_id: formID, submission_id: submissionID, action: 'erforms_get_form_for_edit'}, function (res) {
                if (res.success) {
                    if (res.data.hasOwnProperty('form_html')) {
                        ...
                        if(res.data.submission && res.data.submission.hasOwnProperty('attachments')){
                            var attachments= res.data.submission.attachments;
                            // Looping through attachment fields to generate preview and add delete action.
                            for(var c=0;c<attachments.length;c++){
                                attachFileWithField(submissionContainer,attachments[c],submissionID,true);
                            }
                        }
