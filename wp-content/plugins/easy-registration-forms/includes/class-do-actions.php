...
class ERForms_Do_Actions{
    ...
    public function delete_sub_attachment(){
        ...
        $field_name= sanitize_text_field($_POST['f_name']);
    ...
    public function change_sub_columns(){
        ...
        $columns = array_map(function($item){ return sanitize_text_field($item); },$columns);
        $form = erforms()->form->get_form($form_id);
        if(empty($form))
        {
            wp_send_json_success();
    ...
    public function process_form_html($html,$form){
        ...
        foreach($fields as $field){
            array_push($search,'__'.$field['label'].'__');
            array_push($replace,'<span class="erf_dynamic_ph">__'.$field['label'].'__</span>');
        }
