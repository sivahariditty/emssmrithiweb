            ...
            <?php if($plan['type']=='user'): ?>
                <div class="fb-checkbox-group form-group form-group-plans">
                    <div class="checkbox erf-donation-field">  
                        <label class="fb-text-label"><?php echo $plan['name']; ?><span class="fb-required">*</span></label>
            ...
            <?php elseif($plan['type']=='product'): ?>
                <div class="fb-radio-group fb-checkbox-group form-group form-group-plans">
                    <div class="radio checkbox">
                        ...
                        <input class="erf-price" required   data-plan-type="product" data-erf-price="<?php echo $plan['price']; ?>" type="radio" value="<?php echo $plan['id']; ?>" name="plan_id" id="plan_id_<?php echo $plan['id']; ?>" />
                        <?php else: $initial_amount += $plan['price']; ?>
                        <input class="erf-price" checked required  data-plan-type="product" data-erf-price="<?php echo $plan['price']; ?>" type="checkbox" value="<?php echo $plan['id']; ?>" name="plan_id_<?php echo $plan['id']; ?>" id="plan_id_<?php echo $plan['id']; ?>"/>
                        <?php endif; ?>
                        <label class="fb-text-label"><?php echo $plan['name']; ?><span class="fb-required">*</span></label>
                    </div>
                    <?php if(!empty($plan['description'])): ?>
            ...
            <?php if(is_array($this->options['payment_methods'])) : ?>
                <?php foreach($this->options['payment_methods'] as $payment_method): 
                    ...
                    if(empty($active))
                        continue;
                ?>
                    <input type="radio" checked name="payment_method" value="<?php echo $payment_method; ?>"/> <?php echo $payment_method_label; ?>
                <?php endforeach; ?>
            <?php endif; ?> 
        </div>
</div>  
