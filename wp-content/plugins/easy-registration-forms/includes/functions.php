...
function erforms_sanitize_field_data($field, &$data) {
    ...
    switch ($type) {
        ...
        case 'textarea': $data[$field->name] = isset($_POST[$field->name]) ? sanitize_textarea_field($_POST[$field->name]) : '';
            break;
        case 'number': $data[$field->name] = isset($_POST[$field->name]) ? erforms_get_numeric($_POST[$field->name]) : 0;
            break;
...
function erforms_is_admin_page() {
    $admin_pages = array('erforms-dashboard', 'erforms-overview', 'erforms-submissions', 'erforms-submission', 'erforms-settings','erforms-analytics','erforms-labels','erforms-plans','erforms-plan','erforms-tools');
...
function erforms_text_helpers() {
    $help_texts = array(
        'required-wrap' => 'Prevents submission if this field is empty.',
        ...
        'description-wrap' => 'Show a description of this field.',
        'data-date-format-wrap'=>'Date format for front end.',
        'advance-wrap' => '',
        'subtype-wrap' => 'Field Type',
        'placeholder-wrap' => 'A sample value or a short description of the expected value/format.',
        'className-wrap' => 'Add a custom CSS class.',
        'masking-wrap' => 'Allows fixed width in a certain format (dates,phone numbers, etc). <a href="http://www.easyregistrationforms.com/masked-input-field-using-easy-registration-forms/" target="_blank">More Details</a>',
        'maxlength-wrap' => 'Maximum length of the field.',
        'minlength-wrap' => 'Minimum length of the field.',
        'icon-wrap' => 'Display an icon before the field\'s label.',
        'name-wrap' => '',
        'addUserField-wrap' => 'Map field value with WordPress user meta. <a href="http://www.easyregistrationforms.com/how-to-create-custom-user-meta-fields-with-easy-registration-forms/" target="_blank">More Details</a>',
        'addUserFieldMap-wrap' => 'Meta key(s) to map with User Account in WordPress. Use comma(,) to separate multiple keys.',
        ...
        'pattern-wrap' => 'The pattern attribute specifies a regular expression that the &lt;input&gt; element\'s value is checked against.<a target="_blank" href="https://www.w3schools.com/tags/att_input_pattern.asp">More Details</a>',
        'confirmPassword-wrap' => 'Displays a confirm password field.',
        'user_roles-wrap' => 'Allow to show user role list to pick from. User will be registered under selected role. <br>*Note: Please do not forget to change "Assign User Role" to "Inherit from Form" in Configuration->User Account settings.',
        'other-wrap' => 'This allows to add any custom option from frontend.',
        'inline-wrap' => 'Displays all the options in one line.',
        'dataRefLabel-wrap'=>"This is for refering non input fields (For internal purpose) and will not visible on front end.",
        'dataErfBtnPos-wrap'=>"Select where the button is placed.",
        'enableIntl-wrap'=>'It adds a flag dropdown to the field to detects the user\'s country.<a target="_blank" href="https://www.jqueryscript.net/form/jQuery-International-Telephone-Input-With-Flags-Dial-Codes.html">More Details</a>'
    );
    return $help_texts;
}
...
function erforms_system_notifications(){
    $defaults= array('offline'=>__('Offline User Email','erforms'),'payment_pending'=>__('Payment Pending(User)','erforms'),
                     ...
                     'note_to_user'=>__('Submission Note','erforms'));
