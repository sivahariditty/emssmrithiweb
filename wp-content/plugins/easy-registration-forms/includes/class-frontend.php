...
class ERForms_Frontend {
    ...
    private function register_scripts() {
        wp_register_script('erf-global',ERFORMS_PLUGIN_URL.'assets/js/erforms-global.js',array('jquery'));
        $global_localize=erforms_global_localize();
        wp_localize_script('erf-global','erf_global',$global_localize);
        wp_register_script('erf-util-functions', ERFORMS_PLUGIN_URL . 'assets/js/utility-functions.js');
    ...
    public function register_styles() {
        wp_register_style('erf-front-style', ERFORMS_PLUGIN_URL . 'assets/css/style.css');
        wp_register_style('erf-front-style-responsive', ERFORMS_PLUGIN_URL . 'assets/css/responsive.css');
    ...
    public function render_form($atts) {
        ...
        $this->enqueues($form);
        $form_html = apply_filters('erf_process_form_html',$form['form_html'],$form);
                    ...
                    foreach ($user_field_map as $req_key => $meta_key) {
                        ...
                        if (isset($data[$req_key]) && !$is_primary_key) {
                            ...
                            foreach($m_keys as $m_key){
                                if(!empty($m_key)){
                                    update_user_meta($id, $m_key, $data[$req_key]);
                                }
                            }
                        }
                    }
            ...
            if (!empty($user->ID)) {
                ...
                foreach ($user_field_map as $req_key => $meta_key) {
                    if (isset($data[$req_key])) {
                        ...
                        foreach($m_keys as $m_key){
                            if(!empty($m_key)){
                                update_user_meta($user->ID, $m_key, $data[$req_key]);
                            }
                        }
                    }
                }
    ...
    public function localize_data($form){
        ...
        // User meta or URL params should be prefilled only when we are not loading submission data
        if(empty($submission)){
            $user_meta = erforms()->user->frontend_localize_user_meta($form);
            $filtered_url_params = array();
            foreach ($_GET as $key => $val) {
                $filtered_url_params[urldecode(strtolower(stripslashes($key)))] = sanitize_text_field(stripslashes($val));
            }
