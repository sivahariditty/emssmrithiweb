...
class ERForms_Admin_Submission {
    ...
    */
	...
	$page = isset( $_GET['page'] ) ? $_GET['page'] : '';
        ...
        if (isset($_GET['delete_nonce']) && wp_verify_nonce($_GET['delete_nonce'], 'submission_delete')){
    ...
    */
    public function submission() {
        ...
        if(isset($_GET['dis_edit_sub_nonce']) && wp_verify_nonce($_GET['dis_edit_sub_nonce'], 'disable_edit_submission')){
                 erforms()->submission->update_meta($sub_id,'dis_edit_submission',1);
        }
        
        if(isset($_GET['en_edit_sub_nonce']) && wp_verify_nonce($_GET['en_edit_sub_nonce'], 'enable_edit_submission')){
   ...
   private function save_note($sub_id){
       ...
       if(isset($_POST['add_note'])){
           $data= array();
           $data['text']= wpautop(wp_kses_post(stripslashes($_POST['note_text'])));
           if(!empty($data['text'])){
                if(!empty($_POST['notify_user'])){
                    $note_recipients= !empty($_POST['note_recipients']) ? sanitize_text_field($_POST['note_recipients']) : '';
