...
class ERForms_Form extends ERForms_Post {
    ...
    public function ajax_save_form() {
        ...
        if (is_array($form['fields'])) {
            foreach ($form['fields'] as $index => $form_field) {
                if (empty($form_field['label']) && !empty($form_field['type']) && $form_field['type'] != 'paragraph') {
                ...
                if (isset($form_field['label'])) {
                    if (!empty($form_field['type']) && $form_field['type'] != 'paragraph') {
                        $form_field['label'] = sanitize_text_field($form_field['label']);
                    }
                    $form['fields'][$index] = $form_field;
                }
    ...
    public function dynamic_rules($form) {
        ...
        foreach ($form['fields'] as $field) {
            ...
            if (!empty($field['subtype']) && $field['subtype'] == 'user_email') {
                array_push($form['dynamic_rules']['change'], array('action' => 'erforms_user_email_change', 'field_name' => $field['name']));
            }
        }
    ...
    public function frontend_localize_data($form) {
        ...
        if (isset($form['conditions'])) {
            ...
            if (isset($form_attr_data['conditions']['notifications'])) {
                unset($form_attr_data['conditions']['notifications']);
            }
        }
        $data['form'] = $form_attr_data;
