	...
	$.wpEditor = {
		...
		addClassOnHeader: function( template ){
			if( ! jQuery('body').hasClass('post-type-page') ) {
				$('body').removeClass('shop-isle-header-template');
				return;
			}
			...
			$('body').addClass('shop-isle-header-template');
		},
		...
		addSidebarOnPage: function(template){
			...
			if( $('.editor-block-list__layout').parent().hasClass('shop-isle-editor-wrapper')){
				return;
			}
			$('.editor-block-list__layout').wrap( '<div class="shop-isle-editor-wrapper"></div>');
		...
		addSidebarOnPost: function(){
			...
			$('.editor-post-title').parent().parent().wrap('<div class="shop-isle-editor-wrapper"></div>');
			$('.shop-isle-editor-wrapper').append( this.getSidebarMarkup() );
		},
