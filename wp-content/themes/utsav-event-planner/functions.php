...
function utsav_event_planner_scripts() {
	...
	wp_enqueue_script( 'utsav-event-planner-skip-link-focus-fix', get_theme_file_uri( '/assets/js/skip-link-focus-fix.js' ), array(), '1.0', true );

	$utsav_event_planner_l10n=array();
	
	if ( has_nav_menu( 'top' ) ) {
		wp_enqueue_script( 'utsav-event-planner-navigation-jquery', get_theme_file_uri( '/assets/js/navigation.js' ), array( 'jquery' ), '1.0', true );
		$utsav_event_planner_l10n['expand']         = __( 'Expand child menu', 'utsav-event-planner' );
		$utsav_event_planner_l10n['collapse']       = __( 'Collapse child menu', 'utsav-event-planner' );		
	}
	...
	wp_localize_script( 'utsav-event-planner-skip-link-focus-fix', 'utsav_event_plannerScreenReaderText', $utsav_event_planner_l10n );
...
//footer Link
define('UTSAV_EVENT_PLANNER_CREDIT','https://www.luzuk.com/','utsav-event-planner');

if ( ! function_exists( 'utsav_event_planner_credit' ) ) {
	function utsav_event_planner_credit(){
		echo "<a href=".esc_url(UTSAV_EVENT_PLANNER_CREDIT)." target='_blank'>".esc_html__('Luzuk','utsav-event-planner')."</a>";
	}
}
